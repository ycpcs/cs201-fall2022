...
function generateDates() {
    ...
    while (dateIter <= semesterInfo.lastDayOfSemester) {
        if (courseInfo22.classDays.myincludes(days[dateIter.getDay()])) {
...
function populateFinalExams() {
    if (!courseInfo2.inClassFinalExam) {
        for (var i=0; i < courseInfo2.finalExamDates.length; i++) {
            calendar.push(
                {
                    date: courseInfo2.finalExamDates[i].date,
                    topic: courseInfo2.finalExamDates[i]
                });
        }
    }
}
...
function getRegularSemesterDays() {
    return calendar.length - ((courseInfo2.inClassFinalExam)? 0 : courseInfo2.finalExamDates.length);
}
...
function populateCalendar() {
    ...
    for (var calendarIndex=0, topicIndex=0; calendarIndex < regularSemesterDays; calendarIndex++) {
        if ((calendar[calendarIndex].topic === undefined) && (courseInfo2.classPeriods[topicIndex] !== undefined)) {
            calendar[calendarIndex].topic = courseInfo2.classPeriods[topicIndex].topic;
            // calendar[calendarIndex].reading = courseInfo2.classPeriods[topicIndex].reading;
            calendar[calendarIndex].lab = courseInfo2.classPeriods[topicIndex].lab;
            calendar[calendarIndex].assign = courseInfo2.classPeriods[topicIndex].assign;
            calendar[calendarIndex].reading = courseInfo2.classPeriods[topicIndex].reading;
            topicIndex++;
        }
    }
...
function printCalendar(opts) {
    ...
    if (opts.omitFinalExams && !courseInfo2.inClassFinalExam) {
        numItems -= courseInfo2.finalExamDates.length;
    }
...
/*
        ...
        if (((i !== 0) && (days[calendar[i].date.getDay()] === courseInfo2.classDays[0])) || (!courseInfo2.inClassFinalExam && i === regularSemesterDays)) {
